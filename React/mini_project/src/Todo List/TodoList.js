                ...
                <div style={{ display: "flex", justifyContent: "space-between", flexDirection: "column", width: "100%", backgroundColor: "whitesmoke" }}>
                    <div>
                        ...
                        <ul style={{ listStyle: "none" }} className="todoList">
                            {this.state.todo.map((ele, idx) => <li><span onClick={() => this.edit(idx)}> {ele} </span><img alt="" src={next} className="next" style={{width: "20px", position: "relative"}} onClick={() => this.move(idx)}></img></li>)}
                        </ul>
                    </div>
                    <div>
                        <input type="text" value={this.state.text} onChange={(e) => this.setState({text: e.target.value})}/><button onClick={this.state.text.length > 0 ? this.addTodo: undefined}>add</button>
                    </div>
                </div>
                <div style={{ backgroundColor: "wheat", height: "100%", width: "100%" }}>
                    <h3 style={{ width: "100%", backgroundColor: "gray" }}>complete</h3>
                    <ul style={{ listStyle: "none" }} className="todoList">
                        {this.state.complete.map((ele, idx) => <li><span onClick={() => this.reverse(idx)}> {ele} </span><img alt="" src={bin} className="bin" style={{width: "20px", position: "relative"}} onClick={() => this.delete(idx)}></img></li>)}
                    </ul>
                </div>
            </div>
        )
